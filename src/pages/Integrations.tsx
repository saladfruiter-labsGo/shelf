 ...
 * IsThereAnyDeal). Configurações ficou só com as preferências do app.
 */
export function Integrations() {
  return (
    <div className="px-6 py-8 max-w-2xl">
      <div className="mb-8">
        <h1 className="font-display text-3xl font-bold text-primary mb-1">Integrações</h1>
...
function IntegrationsSection() {
  ...
  const steamTest = useMutation({
    mutationFn: api.integrations.steamTest,
    onSuccess: (d) => {
      const owned = d.owned != null ? ` · ${d.owned} jogos na biblioteca` : ''
      const write = d.can_write ? ' · escrita liberada' : ' · só leitura (sem cookies)'
      // A Steam responde igual para wishlist vazia e perfil privado.
      const wishlist = d.wishlist
        ? `${d.wishlist} itens na wishlist`
        : 'wishlist vazia (ou o perfil não está público)'
      setMsg(`Steam respondeu: ${wishlist}${owned}${write}.`)
      setTimeout(() => setMsg(''), 5000)
    },
    onError: (e: unknown) => { setMsg('Falha no teste: ' + ((e as Error).message || '')); setTimeout(() => setMsg(''), 5000) },
  })
  const steamSync = useMutation({
    mutationFn: api.integrations.steamSync,
    onSuccess: (r) => {
      qc.invalidateQueries({ queryKey: ['integrations'] })
      qc.invalidateQueries({ queryKey: ['media-all'] })
      const parts = [`${r.pulled} vindo(s) da Steam`, `${r.pushed} enviado(s)`]
  ...
  const steamResolve = useMutation({
    mutationFn: () => api.integrations.steamResolve(String(form.steam_id ?? '')),
    onSuccess: (d) => {
      if (d.steam_id) { setForm(f => ({ ...f, steam_id: d.steam_id! })); setMsg('SteamID resolvido — salve para aplicar.') }
  ...
  return (
    <div className="mt-12">
      <div className="mb-6">
        <h2 className="font-display text-2xl font-bold text-primary mb-1">Serviços conectados</h2>
        <p className="text-muted text-sm">Monitore automaticamente o que você assiste no Plex, ouve no YouTube Music, lê no Kavita, joga no Playnite e quer comprar na Steam</p>
      </div>
      ...
      <div className="bg-surface border border-border rounded-xl p-5 mb-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${status?.steam.cookie_set ? 'bg-games-bg text-games' : 'bg-card text-muted'}`}>
              {status?.steam.cookie_set ? 'Leitura e escrita' : 'Só leitura'}
            </span>
          </div>
          <Toggle on={!!form.steam_enabled} onChange={set('steam_enabled')} />
        </div>
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => steamResolve.mutate()} disabled={steamResolve.isPending || !form.steam_id}
                className="px-3 py-2 bg-card border border-border rounded-lg text-xs text-primary hover:border-accent transition-colors whitespace-nowrap disabled:opacity-50">
                {steamResolve.isPending ? '…' : 'Resolver'}
              </button>
            </div>
            <p className="text-[11px] text-muted mt-1">
              O perfil precisa estar público (Perfil → Privacidade → <span className="text-secondary">Detalhes do jogo: Público</span>).
            </p>
          </div>
          ...
          <div>
            ...
            <a href="https://steamcommunity.com/dev/apikey" target="_blank" rel="noopener noreferrer"
               className="text-[11px] text-accent hover:underline">Gerar a chave →</a>
          </div>
        ...
        {/* Cookies da loja — habilitam o sentido Shelf → Steam */}
        <details className="mt-4 bg-card border border-border rounded-lg p-3">
          <summary className="text-xs text-secondary cursor-pointer">
            Cookies da loja <span className="text-muted">— só para enviar do Shelf para a Steam</span>
          </summary>
          <div className="space-y-3 mt-3">
            <p className="text-[11px] text-muted">
              No navegador em que você está logado na Steam, abra as ferramentas do desenvolvedor →
              <span className="text-secondary"> Application → Cookies → store.steampowered.com</span> e copie os dois valores.
              Eles ficam só no banco do seu servidor e expiram quando você sai da conta na Steam.
            </p>
            <div>
        ...
        <div className="flex gap-2 mt-4">
          <button type="button" onClick={() => steamTest.mutate()} disabled={steamTest.isPending || !status?.steam.steam_id}
            className="text-xs px-3 py-2 bg-card border border-border rounded-lg text-primary hover:border-accent transition-colors disabled:opacity-50">
            {steamTest.isPending ? 'Testando…' : '⚡ Testar conexão'}
          </button>
        ...
        {status?.steam.last_sync ? (
          <p className="text-[11px] text-muted mt-2">
            Última sincronização há {timeAgo(status.steam.last_sync.at)} · {status.steam.last_sync.pulled} da Steam ·
            {' '}{status.steam.last_sync.pushed} para a Steam
            {status.steam.last_sync.pending_push > 0 && ` · ${status.steam.last_sync.pending_push} esperando os cookies`}
            {status.steam.last_sync.unmatched.length > 0 && ` · ${status.steam.last_sync.unmatched.length} sem AppID`}
            {status.steam.last_sync.errors.length > 0 && (
              <span className="text-movies"> · {status.steam.last_sync.errors[0]}</span>
            )}
          </p>
        ) : (

...
#
# Quando envia:
#   1. Todo dia às 21h, varre a biblioteca inteira e envia (com as notas atuais).
#      Precisa do Playnite aberto às 21h. Se estiver fechado, faz o envio atrasado
#      na próxima vez que você abrir o Playnite (se já passou das 21h e não sincronizou
#      naquele dia).
#   2. Ao fechar um jogo, envia aquele jogo após 30 segundos (pega tempo/status na hora;
#      a nota que você der depois entra no envio das 21h).
#
# Configuração: crie um arquivo "config.json" nesta mesma pasta com o conteúdo:
#   { "WebhookUrl": "http://SEU-SHELF:3000/api/integrations/playnite/webhook?token=SEU_TOKEN" }
# (copie a URL exata no Shelf → Configurações → Integrações → Playnite → Copiar)
#
# Opcional no config.json: "DailyHour": 21  (hora do envio diário, 0–23; padrão 21)
#
# Um config.json separado sobrevive a atualizações da extensão (o .psm1 pode ser sobrescrito).
...
function Get-ShelfConfig {
    ...
    if (Test-Path $cfgPath) {
        ...
        catch { $__logger.Error("ShelfSync: config.json inválido — $($_.Exception.Message)") }
    }
    return $null
}
...
function Send-ShelfGame {
    ...
    if (-not $url) {
        $__logger.Warn("ShelfSync: WebhookUrl não configurada (crie o config.json). Jogo ignorado: $($game.Name)")
        return
    }
...
function Send-AllGames {
    ...
    try {
        ...
        $__logger.Info("ShelfSync: sincronização enviou $sent jogos")
    } finally {
        $global:ShelfSyncing = $false
    }
    return $sent
...
#   - Durante o dia (timer): só quando já passou da hora do envio (>= 21h) e não sincronizou hoje.
#   - Na abertura do Playnite (isStartup): também sincroniza se um dia inteiro foi pulado
...
function Invoke-DailySyncIfDue {
    ...
    if ($last -eq $today) { return }  # já sincronizou hoje

    $due = $false
    if ($now.Hour -ge (Get-ShelfDailyHour)) {
        $due = $true                                        # já passou da hora, hoje
    } elseif ($isStartup) {
        $yesterday = $now.AddDays(-1).ToString("yyyy-MM-dd")
        if ($last -ne $yesterday) { $due = $true }          # pulou um dia (ou nunca) → catch-up imediato
    }
...
function OnApplicationStarted {
    ...
    # Catch-up: se abriu o Playnite depois da hora, ou se um dia foi pulado, envia agora.
    try { Invoke-DailySyncIfDue $true } catch { $__logger.Error("ShelfSync: catch-up falhou — $($_.Exception.Message)") }

    # Timer que checa a cada minuto se cruzou a hora do envio diário.
    $t = New-Object System.Timers.Timer
    $t.Interval = 60000
    $t.AutoReset = $true
    $t.add_Elapsed({
        param($s, $e)
        try { Invoke-DailySyncIfDue } catch { $__logger.Error("ShelfSync: envio diário falhou — $($_.Exception.Message)") }
    })
    $t.Start()
    $global:ShelfDailyTimer = $t
    $__logger.Info("ShelfSync: timer diário ativo (hora $(Get-ShelfDailyHour)h)")
}
...
# Ao fechar um jogo: envia aquele jogo após 30 segundos.
...
# Item no menu de contexto do jogo: reenvia sob demanda (útil depois de dar nota
# ou marcar como concluído sem esperar o envio diário).
